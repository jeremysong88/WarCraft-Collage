...
public class ChatActivity extends BaseMessageActivity implements OnActiveChatActivityListenner {
	...
	private void init() {
		...
		// 设置top用户信息
		mHeaderLayout.setTitleChat(ImageUtils.getIDfromDrawable(this, Users.AVATAR + mPeople.getAvatar()), R.drawable.bg_chat_dis_active,
				mPeople.getNickname(), mPeople.getLogintime(), R.drawable.ic_topbar_profile, new OnRightImageButtonClickListener());
		// 设置输入控件（表情）
		...
		mClvList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				case VOICE:
					// 播放录音
					final ImageView imgView = (ImageView) view.findViewById(R.id.voice_message_iv_msgimage);
					if (!isPlay) {
				...
				case FILE:
					...
					fileIntent.setType("*/*");
					fileIntent.setData(Uri.parse("file://" + FileUtils.getPathByFullPath(msg.getMsgContent())));
		...
		case ImageUtils.INTENT_REQUEST_CODE_CAMERA:
			if (resultCode == RESULT_OK) {
				if (mCameraImagePath != null) {
					mCameraImagePath = ImageUtils.savePhotoToSDCard(ImageUtils.CompressionPhoto(mScreenWidth, mCameraImagePath, 2),
	...
	public void processMessage(android.os.Message msg) {
		LogUtils.d(TAG, "processMessage>>>> " + msg.what);
		switch (msg.what) {
		...
		case IPMSGConst.IPMSG_RECEIVE_IMAGE_DATA: { // 图片开始发送
			...
			tcpClient.startSend();
			tcpClient.sendFile(mCameraImagePath, mPeople.getIpaddress(), Message.CONTENT_TYPE.IMAGE);
		}
			break;
		...
		case IPMSGConst.IPMSG_RECIEVE_VOICE_DATA: { // 语音开始发送
			...
			if (FileUtils.isFileExists(mVoicePath))
				tcpClient.sendFile(mVoicePath, mPeople.getIpaddress(), Message.CONTENT_TYPE.VOICE);
		}
			break;
		...
		case IPMSGConst.IPMSG_RECIEVE_FILE_DATA: { // 文件开始发送
			...
			if (FileUtils.isFileExists(sendFilePath))
				tcpClient.sendFile(sendFilePath, mPeople.getIpaddress(), Message.CONTENT_TYPE.FILE);
		}
			break;
	...
	private void startRecord() {
		...
		RECORD_FILENAME = System.currentTimeMillis() + szu.wifichat.android.util.TextUtils.getRandomNumStr(3);
